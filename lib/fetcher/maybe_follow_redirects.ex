defmodule Unfurl.Tesla.Middleware.MaybeFollowRedirects do
  @moduledoc """
  Follow HTTP 3xx redirects, based on `Tesla.Middleware.FollowRedirects` with some changes
  ...
  defp redirect(env, next, left) when left == 0 do
    case Tesla.run(env, next) do
      {:ok, %{status: status} = res} when status not in @redirect_statuses ->
        {:ok, res}

      {:ok, res} ->
        warn("Too many redirects, return the last response")
        {:ok, res}
